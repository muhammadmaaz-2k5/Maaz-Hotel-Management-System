...
import { useCancelBookingMutation } from "../store/apiSlice";
  ...
  const handleCancel = async () => {
    try {
      const data = await cancelBooking({
        ...
        payload: { cancellationReason: reason.trim() || undefined },
      }).unwrap();
      ...
      const message =
        err.data?.message || err.message || "Could not cancel booking";
      showToast({
        title: "Cancel failed",
        description: message,
        type: "ERROR",
