import { useGetCurrentUserQuery, useGetHotelByIdQuery, useCreatePaymentIntentMutation } from "../store/apiSlice";
...
const Booking = () => {
  ...
  const [createPaymentIntent, { data: paymentIntentData, isLoading: isLoadingPayment }] = useCreatePaymentIntentMutation();

  useEffect(() => {
    if (hotelId && numberOfNights > 0) {
      createPaymentIntent({ hotelId, numberOfNights: numberOfNights.toString() });
    }
  }, [hotelId, numberOfNights, createPaymentIntent]);

  const { data: hotel, isLoading: isLoadingHotel } = useGetHotelByIdQuery(hotelId as string, {
    skip: !hotelId,
  });
