...
const AnalyticsDashboard = () => {
  ...
  const avgConf = forecastData?.forecasts?.length
    ? forecastData.forecasts.reduce((s: number, f: any) => s + (f.confidence || 0), 0) /
  ...
  const topCategory =
    categoryRows.length > 0
      ? categoryRows.reduce((a: any, b: any) => (b.score > a.score ? b : a))
      : null;

  const statusChartData =
    analyticsData?.bookingStatusBreakdown?.map((r: any) => ({
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    ...
                    dataKey="count"
                  >
                    {analyticsData.popularDestinations.map((_: any, index: number) => (
                      <Cell
                        key={`dest-${index}`}
              ...
              <div className="overflow-x-auto">
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {analyticsData.popularDestinations.map((dest: any) => (
          ...
          <div className="bg-white rounded-xl shadow-xl border p-6 w-full">
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {analyticsData.hotelPerformance.slice(0, 10).map((hotel: any) => (
                    <tr
